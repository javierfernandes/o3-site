...
```scala
@Inject extension StringExtensionProvider
```
...
* Sin embargo nos gustaría invocar este método `publicar()` sobre estos mismos objetos.
...
class AtencionAlClienteExtension {
  ...
  def dispatch atender(ClienteRaso raso) {
    println("ClienteRaso: L@ hacemos esperar su turno")
  }
 
  def dispatch atender(ClienteVIP vip) {
    println("ClienteVIP: L@ atendemos inmediatamente y le damos un Martini y un habano")
  }
...
* queremos **agregar un nuevo método a una jerarquía** ya existente de `Evento`'s (volvemos al ejemplo de los `Medio`s)
...
Están en el [SVN de la materia](https://xp-dev.com/svn/uqbar/examples/paco/trunk/languages/xtend/xtend-examples)
